...
* **/Sample_Data:** Contains 50 representative synthetic video clips generated from the UE5 pipeline, alongside their corresponding ground truth and prediction JSON vectors.
  * `/Videos`: 50 representative `.mp4` video clips.
  * `/Ground_Truth_and_Predictions`: 50 matching `.json` files containing the evaluation vectors.
* **/Scripts:** Contains the core logic required to reproduce the VLM evaluation pipeline and the cross-validation splits.
